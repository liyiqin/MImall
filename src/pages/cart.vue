<template>
  <div class="cart">
    ...
    <!--自定义的弹框使用<modal title="提示" sureText="确定" btnType="3" modalType="middle" :showModal="showModal"
      @submit="deleteList" @cancel="showModal=false">
...
export default {
  ...
  methods: {
    //自定义弹框确定时删除商品
    /* deleteList() {
      this.axios.delete(`/carts/${this.op}`).then((res) => {
        this.renderData(res);
    ...
    delProduct(item) {
      this.axios.delete(`/carts/${item.productId}`).then((res) => {
        /*自定义弹框变量赋值 this.op = item.productId; */
